...
if (!$dblink) {
    echo "<meta http-equiv='content-type' content='text/html; charset={$g4['charset']}'>";
    echo "<script type='text/javascript'>alert('MySQL Host, User, Password 를 확인해 주십시오.');history.back();</script>";
    exit;
}
...
if (!$select_db) {
    echo "<meta http-equiv='content-type' content='text/html; charset={$g4['charset']}'>";
    echo "<script type='text/javascript'>alert('MySQL DB 를 확인해 주십시오.');history.back();</script>";
    exit;
}
?>
<html>
...
echo "<script type='text/javascript'>document.frminstall2.job1.value='전체 테이블 생성중';</script>";
flush(); usleep(50000);
...
echo "<script type='text/javascript'>document.frminstall2.job1.value='전체 테이블 생성 완료';</script>";
flush(); usleep(50000);
...
fclose($f);
@chmod($file, 0606);
echo "<script type='text/javascript'>document.frminstall2.job3.value='DB설정 파일 생성 완료';</script>";
...
echo "<script type='text/javascript'>document.frminstall2.job4.value='필요한 Table, File, 디렉토리 생성을 모두 완료 하였습니다.';</script>";
echo "<script type='text/javascript'>document.frminstall2.job5.value='* 메인화면에서 운영자 로그인을 한 후 운영자 화면으로 이동하여 환경설정을 변경해 주십시오.';</script>";
flush(); usleep(50000);
?>

<script type='text/javascript'>document.frminstall2.btn_next.disabled = false;</script>
<script type='text/javascript'>document.frminstall2.btn_next.focus();</script>
