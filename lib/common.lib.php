...
function explain($sql)
{
    if (preg_match("/^(select)/i", trim($sql))) {
        ...
        echo " <span style='color:#00f'>(type={$row['type']} , key={$row['key']})</span>";
    }
}
...
function isnot($val=null, $pre=null) {
	return $val ? $val : $pre;
}

function isget($val=null, $key=null, $def=null) {
	if (is_null($val)) return isnot($def, null);
	switch (gettype($val)) {
		case 'string':
			return isset($GLOBALS[$val]) ? $GLOBALS[$val] : $key;
		case 'array':
			return isset($val[$key]) ? $val[$key] : $def;
		case 'object':
			if (is_callable(array($val, $key)))
				return $val->{$key}();
