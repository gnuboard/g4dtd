...
function get_member_id_select($name, $level, $selected='', $event='')
{
    ...
    $str = "<select name='$name' $event><option value=''>선택안함</option>";
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= "<option value='{$row['mb_id']}'";
        if ($row['mb_id'] == $selected) $str .= " selected='selected'";
...
function textarea_size($fld)
{
    ...
    $s  = "<div>";
    $s .= "<div class='fr'>";
    $s .= "<span onclick=\"javascript:textarea_size(document.getElementById('$fld'), {$size})\"><img src='{$g4['admin_path']}/img/btn_up.gif' alt='' /></span> ";
    $s .= "<span onclick=\"javascript:textarea_size(document.getElementById('$fld'), ".$size*(-1).")\"><img src='{$g4['admin_path']}/img/btn_down.gif' alt='' /></span>";
    $s .= "</div>";
    $s .= "</div>";
    return $s;
}
...
function icon($act, $link="", $target="_parent")
{
    ...
    $icon = "<img src='{$g4['admin_path']}/img/icon_{$img[$act]}.gif' alt='$act' title='$act' class='icon_btn' />";
    if ($link)
...
function help($help="", $left=0, $top=0)
{
    ...
    $str  = "<img src='{$g4['admin_path']}/img/icon_help.gif' class='icon_help' alt='도움말' onclick=\"help('help$idx', $left, $top);\" style='cursor:hand;' />";
    ...
    $str .= "<div class='csshelp1'><div class='csshelp2'><div class='csshelp3'>$help</div></div></div>";
    $str .= "</div>";
...
function subtitle($title, $more="")
{
    ...
    $s .= "<h3 class='fl'><img src='{$g4['admin_path']}/img/icon_title.gif' width='20' height='9' alt='' /> $title</h3>";
    if ($more)
        $s .= "<p class='fr'><a href='$more'><img src='{$g4['admin_path']}/img/icon_more.gif' width='43' height='11' alt='' /></a></p>";
    $s .= "</div>\n";
