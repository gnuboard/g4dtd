...
if ($is_admin && $w != "r")
{
    ...
    if ($w == "u")
    {
        ...
        else
        {
            ...
            //if (preg_match("/[^0-9]{0,1}{$wr_id}[\r]{0,1}/",$board[bo_notice]))
            if (in_array((int)$wr_id, $notice_array))
                $notice_checked = "checked='checked'";
        }
    }
}
...
if ($w == "" || strstr($write[wr_option], "mail"))
    $recv_email_checked = "checked='checked'";
...
if ($w == "") {
    ...
    $file = get_file($bo_table, $wr_id, $board);
} else if ($w == "r") {
    if (strstr($write[wr_option], "secret")) {
        $is_secret = true;
        $secret_checked = "checked='checked'";
...
if ($is_notice || $is_html || $is_secret || $is_mail) {
    $option = "";
    if ($is_notice) {
        $option .= "<label><input type='checkbox' name='notice' id='notice' value='1' $notice_checked />공지</label>\n";
    }

    if ($is_html) {
        if ($is_dhtml_editor) {
            $option_hidden .= "<input type='hidden' name='html' id='html1' value='html1' />";
        } else {
            $option .= "<label><input type='checkbox' name='html' id='html' value='$html_value' $html_checked />html</label>\n";
        }
    }

    if ($is_secret) {
        if ($is_admin || $is_secret==1) {
            $option .= "<label><input type='checkbox' name='secret' id='secret' value='secret' $secret_checked />비밀글</label>\n";
        } else {
    ...
    if ($is_mail) {
        $option .= "<label><input type='checkbox' name='mail' id='mail' value='mail' $recv_email_checked />답변메일받기</label>\n";
    }
}
